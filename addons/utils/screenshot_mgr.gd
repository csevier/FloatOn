...
func _get_setting(name: String):
	if ProjectSettings.has_setting(name):
		return _get_setting(name)
	return null

func _init_settings():
	var temp = _get_setting("global/screenshot_mgr_max_images")
	if temp and temp > 0:
		_screenshot_max = temp
	
	temp = _get_setting("global/screenshot_mgr_resize_factor")
	if temp and temp > 0.0:
		_resize_factor = temp

	temp = _get_setting("global/screenshot_mgr_toggle_action_name")
	if temp:
		_screenshot_action_name = temp
	
	temp = _get_setting("global/screenshot_mgr_frame_delay")
	if temp:
		_frame_delay = temp
